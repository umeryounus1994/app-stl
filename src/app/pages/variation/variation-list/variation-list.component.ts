...
export class VariationListComponent implements OnInit {
  ...
  openAddModal() {
    const modalRef = this.modalService.open(VariationAddComponent);
  ...
  _openEditModal(variationData) {
    const modalRef = this.modalService.open(VariationEditComponent);
  ...
  deleteVariation(variationId) {
    ...
    Swal({
      ...
      formData.append('state', "deleted");
        this.spinner.show();
        this.api.patch('variation/update/',variationId, formData)
        .then((response: any) => {
          if (response.status === true) {
            this.spinner.hide();
            this.helper.successToast('Success', "Variation Deleted Successfully");
            this.getVariations();
          } else {
            this.spinner.hide();
