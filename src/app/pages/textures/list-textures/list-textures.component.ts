...
@Component({
  selector: 'app-list-textures',
  templateUrl: './list-textures.component.html',
  styleUrls: ['./list-textures.component.scss']
})
export class ListTexturesComponent implements OnInit {
  isDataLoaded = false;
  userId;
  variationsList = [];
  ...
  getVariations() {
    this.api.get('texture/get_all').then((response: any) => {
  ...
  openAddModal() {
    const modalRef = this.modalService.open(AddTexturesComponent);
  ...
  _openEditModal(variationData) {
    const modalRef = this.modalService.open(EditTexturesComponent);
